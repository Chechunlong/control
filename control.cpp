...
void Control::setAmplitude(double amplitude) { this->amplitude = amplitude; }

void Control::setAuxForRand(double auxForRand) { this->auxForRand = auxForRand; }

void Control::setCanalEscrita(int canalEscrita) { this->canalEscrita = canalEscrita; }
...
double Control::readNivel(int canal)
{
    double readVoltage, nivel;

    readVoltage = quanser->readAD(canal);
    nivel = readVoltage * FATOR_CONVERSAO;
...
void Control::sendSignal()
{
    ...
    if(ordemSistema == SISTEMA_ORDEM_1) canalEscrita = 0;
    else canalEscrita = 1;
...
void Control::receiveSigal() {
    ...
    for(int canal=0; canal<2; canal++) {
        if(simulacao) {
            if(canal==0) canaisLeitura_value[canal] = tanq->getNivelTq1();
            else if(canal==1) canaisLeitura_value[canal] = tanq->getNivelTq2();
        } else {
            canaisLeitura_value[canal] = readNivel(canal);
        }
