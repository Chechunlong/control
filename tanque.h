...
public:
    ...
    double getNivelTq1() { return this->nivelTq1; }
    double getNivelTq2() { return this->nivelTq2; }
    double acionaBomba(double sinal) {
        ...
        if(nivelTq1 <= nivelMinimo && sinal < 0) sinal = 0;


        if(this->nivelTq1 > nivelMaximo && sinal > 0) sinal = 0;

        if(this->nivelTq1 > nivelCritico) sinal = 0;
